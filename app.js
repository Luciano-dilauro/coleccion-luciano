...
function sanitizeData(data) {
  ...
  const out = (data && typeof data === "object") ? data : { collections: [] };

  if (!out.collections) out.collections = [];
...
function goLoadEdit() {
  ...
  if (els.deletePicker) els.deletePicker.classList.add("hidden");
...
function renderDetail() {
  ...
  for (const sec of col.sections) {
    ...
    if (state.filter === "miss" && items.length === 0) continue;
...
function renderChips(list, color = "gray") {
